package com.example.allinmarket.domain.notification.restocknotification.sender;
...
public class RestockNotificationSender {
    ...
    public void send(RestockSubscription subscription, Long productId) {
        ...
        subscription.send();
        RestockNotificationEvent event = RestockNotificationEvent.from(notification);
        TransactionSynchronizationManager.registerSynchronization(
                new TransactionSynchronization() {
                    @Override
                    public void afterCommit() {
                        redisPublisher.publishRestockEvent(event);
                    }
                }
        );
    }
