...
class RestockNotificationControllerTest {
    ...
    @Autowired
    private RestockNotificationController controller;

    private MockMvc mockMvc;

    @MockitoBean
    private BuyerRestockNotificationService buyerRestockNotificationService;
    ...
    @Nested
    @DisplayName("GET /restock-notifications/me - 알림 목록 조회")
    class GetNotificationsTest {

        @Test
        @DisplayName("읽지 않은 재입고 알림 목록을 페이지로 반환한다")
        void getNotifications_success() throws Exception {
            // given
            RestockNotificationDetailResponse notification = new RestockNotificationDetailResponse(1L, 200L);
            PageResponse<RestockNotificationDetailResponse> pageResponse = new PageResponse<>(
                    List.of(notification), 1, 1, 1L, 10, true
            );
            given(buyerRestockNotificationService.getNotifications(eq(1L), any(Pageable.class)))
                    .willReturn(pageResponse);

            // when & then
            mockMvc.perform(RestDocumentationRequestBuilders.get("/restock-notifications/me")
                            .param("page", "0")
                            .param("size", "10"))
                    ...
                    .andDo(document("restock-notifications/get-list",
                            ...
                            responseFields(
                                    ...
                                    fieldWithPath("data.content[].productId").type(NUMBER).description("상품 ID"),
        ...
        void getNotifications_empty() throws Exception {
            // given
            PageResponse<RestockNotificationDetailResponse> emptyPage = new PageResponse<>(
            ...
            mockMvc.perform(RestDocumentationRequestBuilders.get("/restock-notifications/me"))
                    ...
                    .andDo(document("restock-notifications/get-list-empty",
                            preprocessResponse(prettyPrint()),
                            responseFields(
                                    fieldWithPath("success").type(BOOLEAN).description("성공 여부"),
                                    fieldWithPath("status").type(NUMBER).description("HTTP 상태 코드"),
        ...
        void getNotifications_buyerNotFound() throws Exception {
            ...
            mockMvc.perform(RestDocumentationRequestBuilders.get("/restock-notifications/me"))
                    ...
                    .andDo(document("restock-notifications/get-list-buyer-not-found",
                            preprocessResponse(prettyPrint()),
                            responseFields(
                                    fieldWithPath("success").type(BOOLEAN).description("성공 여부"),
                                    fieldWithPath("status").type(NUMBER).description("HTTP 상태 코드"),
    ...
    @Nested
    @DisplayName("PUT /restock-notifications/me - 전체 알림 읽음 처리")
    class ReadAllNotificationsTest {

        @Test
        @DisplayName("모든 재입고 알림을 읽음 처리하고 200을 반환한다")
        void readAllNotifications_success() throws Exception {
            // given
            willDoNothing().given(buyerRestockNotificationService).readAllNotifications(1L);

            // when & then
            mockMvc.perform(RestDocumentationRequestBuilders.put("/restock-notifications/me"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
                    .andExpect(jsonPath("$.status").value(200))
                    .andExpect(jsonPath("$.message").value(SuccessEnum.UPDATE_SUCCESS.getMessage()))
                    .andDo(document("restock-notifications/read-all",
                            preprocessResponse(prettyPrint()),
                            responseFields(
                                    fieldWithPath("success").type(BOOLEAN).description("성공 여부"),
                                    fieldWithPath("status").type(NUMBER).description("HTTP 상태 코드"),
        ...
        void readAllNotifications_buyerNotFound() throws Exception {
            // given
            willThrow(new BaseException(ErrorEnum.BUYER_NOT_FOUND))
                    .given(buyerRestockNotificationService).readAllNotifications(1L);
            ...
            mockMvc.perform(RestDocumentationRequestBuilders.put("/restock-notifications/me"))
                    ...
                    .andExpect(jsonPath("$.status").value(ErrorEnum.BUYER_NOT_FOUND.getStatus()))
                    .andExpect(jsonPath("$.message").value(ErrorEnum.BUYER_NOT_FOUND.getMessage()))
                    .andDo(document("restock-notifications/read-all-buyer-not-found",
                            preprocessResponse(prettyPrint()),
                            responseFields(
                                    fieldWithPath("success").type(BOOLEAN).description("성공 여부"),
                                    fieldWithPath("status").type(NUMBER).description("HTTP 상태 코드"),
    ...
    @Nested
    @DisplayName("PUT /restock-notifications/{productId} - 단건 알림 읽음 처리")
    class ReadNotificationTest {

        @Test
        @DisplayName("특정 상품의 재입고 알림을 읽음 처리하고 200을 반환한다")
        void readNotification_success() throws Exception {
            // given
            willDoNothing().given(buyerRestockNotificationService).readNotification(1L, 200L);

            // when & then
            mockMvc.perform(RestDocumentationRequestBuilders.put("/restock-notifications/{productId}", 200L))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
                    .andExpect(jsonPath("$.status").value(200))
                    .andExpect(jsonPath("$.message").value(SuccessEnum.UPDATE_SUCCESS.getMessage()))
                    .andDo(document("restock-notifications/read-one",
                            preprocessResponse(prettyPrint()),
                            pathParameters(
                                    parameterWithName("productId").description("읽음 처리할 상품 ID")
                            ),
                            responseFields(
                                    fieldWithPath("success").type(BOOLEAN).description("성공 여부"),
                                    fieldWithPath("status").type(NUMBER).description("HTTP 상태 코드"),
        ...
        void readNotification_buyerNotFound() throws Exception {
            // given
            willThrow(new BaseException(ErrorEnum.BUYER_NOT_FOUND))
                    .given(buyerRestockNotificationService).readNotification(1L, 200L);
            ...
            mockMvc.perform(RestDocumentationRequestBuilders.put("/restock-notifications/{productId}", 200L))
                    ...
                    .andExpect(jsonPath("$.status").value(ErrorEnum.BUYER_NOT_FOUND.getStatus()))
                    .andExpect(jsonPath("$.message").value(ErrorEnum.BUYER_NOT_FOUND.getMessage()))
                    .andDo(document("restock-notifications/read-one-buyer-not-found",
                            preprocessResponse(prettyPrint()),
                            pathParameters(
                                    parameterWithName("productId").description("읽음 처리할 상품 ID")
                            ),
        ...
        void readNotification_notificationNotFound() throws Exception {
            ...
            willThrow(new BaseException(ErrorEnum.NOTIFICATION_NOT_FOUND))
                    .given(buyerRestockNotificationService).readNotification(1L, 999L);

            // when & then
            mockMvc.perform(RestDocumentationRequestBuilders.put("/restock-notifications/{productId}", 999L))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.success").value(false))
                    ...
                    .andDo(document("restock-notifications/read-one-notification-not-found",
                            preprocessResponse(prettyPrint()),
                            pathParameters(
                                    parameterWithName("productId").description("읽음 처리할 상품 ID")
                            ),
